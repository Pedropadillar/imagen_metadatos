...
Devuelva las palabras clave en español como una lista separada por comas.
Ejemplo:
perro, parque, frisbee, feliz
"""
    ...
    for img_info in image_info_list:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=messages,
                stream=True,
                max_tokens=300 # Limit response length to avoid excessively long outputs
            )
            extracted_keywords = ""
            ...
            q.put({"event": "error", "image_id": image_id, "data": f"API Error: {api_error.message}"})
        except Exception as e:
...
async def stream(task_id: str):
    async def event_generator():
        ...
        while True:
            item = q.get()
            # Format as Server-Sent Event. Include 'id' for client-side e.lastEventId.
            if "image_id" in item:
